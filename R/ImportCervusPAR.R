...
ImportCervusPAR <- function(PARSummaryFile, PARResultsFile, known_parents = TRUE) {
  ...
  for (table in sumstatloc) {
    SummaryStatistics[[paste0(stringr::str_remove(summaryfile[table - 2], pattern = ":"))]] <-
      read.table(
        text = summaryfile[(table + 2):(table + 5)],
        header = FALSE, sep = "\t"
      )
  }
  ...
  if (!known_parents) {
    SummaryStatistics$results$all <- SummaryStatistics$results$all |>
      dplyr::rename(
        ...
        offspring_known_loci_mismatching = "Pair loci mismatching...6",
        offspring_known_lod_score = "Pair LOD score...7",
        offspring_known_delta = "Pair Delta...8",
        offspring_known_confidence = "Pair confidence...9",
        father_id = "Candidate father ID",
        loci_typed_candidate = "Loci typed...11",
        offspring_candidate_loci_compared = "Pair loci compared...12",
        offspring_candidate_loci_mismatching = "Pair loci mismatching...13",
        offspring_candidate_lod_score = "Pair LOD score...14",
        offspring_candidate_delta = "Pair Delta...15",
        offspring_candidate_confidence = "Pair confidence...16",
        offspring_candidate_known_loci_compared = "Trio loci compared",
        offspring_candidate_known_loci_mismatching = "Trio loci mismatching",
        offspring_candidate_known_lod_score = "Trio LOD score",
        offspring_candidate_known_delta = "Trio Delta",
